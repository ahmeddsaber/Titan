...
public class NotificationService : INotificationService
{
    ...
    private readonly IHubContext<TitanHub> _hub;

    // IHubContext<TitanHub> is Singleton (registered by AddSignalR).
    // Injecting Singleton → Scoped is safe and the correct pattern.
    // The old SetHubContext() approach was never called anywhere → hub was always null.
    public NotificationService(ApplicationDbContext db, IHubContext<TitanHub> hub)
    {
        _db  = db;
        _hub = hub;
    }
        ...
        NotificationType type, string? actionUrl = null)
    {
        ...
        // Always non-null — injected via constructor
    ...
    public async Task SendOrderNotificationAsync(Guid userId, string orderNumber, OrderStatus status)
    {
        ...
        await SendNotificationAsync(
            userId, "Order Update 📦", message,
            NotificationType.OrderUpdate, $"/orders/{orderNumber}");
    }
